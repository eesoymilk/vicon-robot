    ...
    try:
        ...
        for deviceName, deviceType in devices:
            ...
            for outputName, componentName, unit in deviceOutputDetails:
                values, occluded = client.GetDeviceOutputValues(
                    deviceName, outputName, componentName
                )
...
def get_vicon_subject_info(client, subjectName):
    print(subjectName)
    segmentNames = client.GetSegmentNames(subjectName)
    for segmentName in segmentNames:
        segmentChildren = client.GetSegmentChildren(subjectName, segmentName)
...
def get_vicon_all_subject_info(client):
    subjectNames = client.GetSubjectNames()
    for subjectName in subjectNames:
        get_vicon_subject_info(client, subjectName)
